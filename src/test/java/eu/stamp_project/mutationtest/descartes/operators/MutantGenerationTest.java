...
public class MutantGenerationTest {
    ...
    public static Collection<Object[]> parameters() {
        return Arrays.asList(new Object[][] { { "null", (Predicate<Object>) Objects::isNull },
                { "this", (Predicate<Object>) Objects::isNull }, { "-1", isEqualTo(-1) },
                { "empty", (Predicate<Object>) (x) -> x.getClass().isArray() && Array.getLength(x) == 0 },
                { "1.2f", isEqualTo(1.2f) }, { "1.0", isEqualTo(1.0) }, { "(short)1", isEqualTo((short) 1) },
                { "(byte)1", isEqualTo((byte) 1) }, { "123L", isEqualTo(123L) }, { "true", isEqualTo(true) },
                { "'\\n'", isEqualTo('\n') }, { "\"\"", isEqualTo("") }, { "\"A\"", isEqualTo("A") },
                { "optional", isEqualTo(Optional.empty()) }, { "new", (Predicate<Object>) Objects::nonNull } // Weak
                                                                                                             // oracle.
                                                                                                             // For the
                                                                                                             // moment
                                                                                                             // we test
    ...
    public void shoultWriteMutant() throws Exception {
        ...
        DescartesMutater mutater = new DescartesMutater(new ClassloaderByteArraySource(target.getClassLoader()),
        ...
        if (mutationPoints.isEmpty())
            ...
            // assertFalse("No mutation point found for mutator: " + operatorID,
            // mutationPoints.isEmpty());
            ...
            for (MutationDetails mutationDetails : mutationPoints) {
                ...
                // Check the result
                // assertTrue("Method <" + mutationDetails.getMethod().name() + "> returned a
                // wrong value for mutation operator: " + operatorID, check.test(result));
            }
    }
