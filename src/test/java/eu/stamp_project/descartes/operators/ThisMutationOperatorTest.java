...
class ThisMutationOperatorTest extends ParameterlessOperatorTest<ThisMutationOperator> {
  ...
  Stream<Method> methodsToMutate() throws NoSuchMethodException {
    return fromClass(Fluent.class).method("addOne").method("addTwo").method("addThree").toStream();
  }
        ...
        .map(
            method ->
                DynamicTest.dynamicTest(
                    ...
                    () -> {
                      assertTrue(
                          checkFluentAnEffect(Fluent.class, method),
                          "Original fluent input method does not have an effect on the receiver. Check the test input");
                      Class<?> mutant = mutate(method);
                      assertFalse(
                          checkFluentAnEffect(mutant, method),
                          "Mutated method must not have an effect on the receiver");
                    }));
  }

  private boolean checkFluentAnEffect(Class<?> owner, Method method)
    ...
    assertEquals(
        receiver, result, "Fluent invocation should return the same instance as the receiver");
