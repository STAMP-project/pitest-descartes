...
class NewInstanceMutationOperatorTest extends ParameterlessOperatorTest<NewInstanceMutationOperator> {
    ...
    @Test
    void testReturnEmptyList() throws NoSuchMethodException, IllegalAccessException, InstantiationException, InvocationTargetException, IOException {
        Method method = Calculator.class.getDeclaredMethod("getMultipleCalculators", int.class);

        List<?> result = (List<?>)invoke(method);
        assertThat("Original result should not be an empty list. Fix the test input.", result, not(empty()));
