...
/**
 * A mutation operator that replaces the method body by a return instruction whose result is the
 * given constant.
 */
public class ConstantMutationOperator extends MutationOperator {
   ...
   *
   * @param id The identifier for the instance of the operator. It does not have to coincide with
   *     the constant value.
  ...
  public ConstantMutationOperator(String id, Object constant) {
    ...
    // Besides, there could be a scenario in which two different operators can produce the same
    // constant.
    final Class<?> constantType = constant.getClass();
    if (!TypeHelper.isConstantType(constantType)) {
      throw new IllegalArgumentException(
          "Given object of type " + constantType.getName() + " is not a constant");
    }
  ...
  /**
   * Gives access to the constant value used by the operator.
   *
  ...
  /**
   * Returns a value indicating whether the operator can transform the given method. In this case,
   * checks if the return type of the method is the same as the type of the constant value.
   *
  ...
  public boolean canMutate(MethodInfo method) {
    ...
    return methodType.equals(constantType) || methodType.equals(wrapperType());
  }
  ...
  protected void generateCode(MethodInfo method, GeneratorAdapter mv) {
    ...
    if (constant.getClass() != String.class && returnType.equals(constantType)) {
      String descriptor =
          String.format("(%s)L%s;", wrapperType().getInternalName(), returnType.getInternalName());
