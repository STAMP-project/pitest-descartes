...
/**
 * Replaces the method body with a return statement, which generates an instance of the specified
 * return type. <br>
...
@Operator(
    identifier = "new",
    description =
        "Method body replaced by instructions to return an instance "
            + "of the class using a constructor with no parameters")
public class NewInstanceMutationOperator extends MutationOperator {

  private static final Map<Class<?>, Class<?>> ALTERNATIVE_RETURN_CLASS_BY_TYPE =
  ...
  private boolean belongsToJavaPackages(Class<?> givenClass) {
    Package classPackage = givenClass.getPackage();
    if (classPackage == null) {
      return false;
    }
    String[] nameParts = classPackage.getName().split("\\.");
    return nameParts.length >= 1 && "java".equals(nameParts[0]);
  }

  /** Returns the specified class or a class which is assignable from this class. */
  protected static Class<?> getAppropriateReturnClass(MethodInfo method)
      throws ClassNotFoundException {
    return getAppropriateReturnClass(method.getReturnType().getClassName());
  }

  /** Returns the specified class or a class which is assignable from this class. */
  protected static Class<?> getAppropriateReturnClass(String className)
      throws ClassNotFoundException {
  ...
  protected void generateCode(MethodInfo method, GeneratorAdapter mv) {
    ...
    mv.visitMethodInsn(
        Opcodes.INVOKESPECIAL, appropriateReturnType.getInternalName(), "<init>", "()V", false);
