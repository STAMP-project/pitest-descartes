...
public class LiteralParser {
    ...
    public static class Result {
        ...
        private Result(Object value, String error) { this.value = value; this.error = error;  }
    ...
    private Result byteOrShortLiteral() throws IOException {
        ...
        return Result.ok(tokenToNumber(new Token( typeToLiteral(resultType) | getRadix(lookahead.getType()) , prefix + lookahead.getLexeme())));
    }

    public void next() throws IOException { lookahead = lexer.nextToken(); }
    ...
    private Object valueOf(Class<?> type, String value) {
        try {
            ...
            throw new AssertionError("Class " + type.getName() + " expected to have an accessible valueOf(String) method");
        }
    ...
    private Object integerValueOf(Class<?> type, String value, int radix) {
        try {
            ...
            throw new AssertionError("Class " + type.getName() + " expected to have an accessible valueOf(String, int) method");
        }
        catch (InvocationTargetException exc) {
