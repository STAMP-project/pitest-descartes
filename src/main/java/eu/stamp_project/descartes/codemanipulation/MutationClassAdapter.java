...
public class MutationClassAdapter extends BaseClassVisitor {
  ...
  private boolean shouldNotTarget(MethodInfo method) {
    ...
    return !location.getMethodDesc().equals(method.getDescriptor())
        || !location.getMethodName().equals(method.getName());
  }
  ...
  public MethodVisitor visitMethod(
    ...
    final MutationOperator operator = MutationOperator.fromIdentifier(mutation.getMutator());
    return new MethodRewritingVisitor(methodVisitor) {
      @Override
      protected void generateCode() {
        GeneratorAdapter adapter =
            new GeneratorAdapter(mv, method.access, method.name, method.getDescriptor());
