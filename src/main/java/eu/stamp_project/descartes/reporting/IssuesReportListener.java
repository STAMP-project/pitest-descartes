...
public class IssuesReportListener implements MutationResultListener {
  ...
  public void handleMutationResult(ClassMutationResults classMutationResults) {
    ...
    MethodRecord[] issues =
        MethodRecord.getRecords(classMutationResults)
    ...
    try (Writer writer = createFile(className + ".html")) {
      ...
      throw Unchecked.translateCheckedException(
          "Could not write file for class: " + className, exc);
    }
  ...
  private String getStringTime() {
    ...
    long[] parts = {
      elapsed.toDays(), elapsed.toHours() % 24, elapsed.toMinutes() % 60, elapsed.getSeconds() % 60
    };
  ...
  public void runEnd() {
    ...
    context.put(
        "total",
        findings.stream()
            .mapToInt(ClassIssues::getIssues)
            .sum()); // Is there a way to put this code into the template??
